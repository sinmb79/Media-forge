...
import {
  type DashboardEditTool,
  getDashboardEditMissingInputs,
  getDashboardEditRequiredBackends,
  isDashboardEditTool,
  normalizeDashboardEditRequest,
  runDashboardEditTool,
} from "@/lib/mediaforge-edit";
...
export async function POST(request: Request, context: { params: Promise<{ tool: string }> }) {
  ...
  if (!isDashboardEditTool(tool)) {
    return NextResponse.json(
      {
        action: "edit-run",
        status: "blocked",
        reason: `Unsupported edit tool: ${tool}.`,
        missing_backends: [],
        missing_inputs: [],
        next_steps: ["Choose a supported edit tool and retry."],
      },
      { status: 422 },
    );
  }

  const normalizedRequest = normalizeDashboardEditRequest(tool, payload);
  const missingInputs = getDashboardEditMissingInputs(normalizedRequest);
  ...
  const missingBackends = getDashboardEditRequiredBackends(tool, normalizedRequest.inputPath)
  ...
  queueMicrotask(() => {
    void executeEdit(jobId, tool, payload);
  });
  ...
  async function executeEdit(
    ...
    try {
      ...
      const result = await runDashboardEditTool(editTool, input, runtime.rootDir);
      const artifactStat = await stat(result.output_path);
      if (artifactStat.size <= 0) {
        throw new Error("Edit action created an empty output file.");
      }
      ...
      runtime.jobQueue.updateMetadata(createdJobId, {
        ...
        expected_artifact: true,
        next_step: "Review the processed clip in Preview Workspace.",
        phase: "verification",
        result_kind: "file",
      ...
      runtime.jobQueue.fail(createdJobId, message, {
        details: message,
        expected_artifact: true,
        next_step: "Check the selected source, mask, or backend setup and retry.",
        phase: "execution",
        result_kind: "file",
